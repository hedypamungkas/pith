...
`@pith/core` is the embeddable engine. It runs with **zero infrastructure and zero API keys by default** (in-memory ports, in-process queue). Opt into persistence, LLM extraction, search, and HTTP/MCP faces as you need them.

- **Three faces:** the SDK (`createEngine`), `@pith/core/http` (Fastify `/v1/*`), `@pith/core/mcp` (5 MCP tools).
